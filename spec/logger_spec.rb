  ...
  describe Logger do
    ...
    describe "#entry_to_hash" do
       ...
       it "should handle a statistics entry" do
         ...
         actual = Appstats::Logger.entry_to_hash("0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address_search")
         actual.should == expected
       end
       
       it "should handle contexts" do
         expected = { :action => "address_filter", :timestamp => "2010-09-21 23:15:20", :server => "Live", :app_name => 'Market' }
         actual = Appstats::Logger.entry_to_hash("0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address_filter : app_name=Market : server=Live")
         actual.should == expected
       end
    
       it "should handle actions with the delimiter (and change the delimiter)" do
         expected = { :action => "address:=search-n", :timestamp => "2010-09-21 23:15:20" }
         actual = Appstats::Logger.entry_to_hash("0.2.0 setup[::,==,--n] 2010-09-21 23:15:20 action==address:=search-n")
         actual.should == expected
       ...
       it "should handle contexts with the delimiter (and change the delimiter)" do
         ...
         actual = Appstats::Logger.entry_to_hash("0.2.0 setup[::,==,--n] 2010-09-21 23:15:20 action==address :: server==market:eval=-n")
         actual.should == expected
       end
    ...
    describe "#entry_to_s" do
      ...
      it "should handle a statistics entry" do
        expected = "0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address_search"
        actual = Appstats::Logger.entry_to_s("address_search")
        actual.should == expected
      end
      
      it "should handle numbers" do
        expected = "0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=1 : note=2.2"
      ...
      it "should handle default contexts" do
        ...
        expected = "0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address_search : app_name=market"
        actual = Appstats::Logger.entry_to_s("address_search")
        actual.should == expected
      end
      
      it "should handle contexts (and sort them by symbol)" do
        expected = "0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address_filter : app_name=Market : server=Live"
        actual = Appstats::Logger.entry_to_s("address_filter", { :server => "Live", :app_name => 'Market' })
        actual.should == expected
      end

      it "should handle actions with the delimiter (and change the delimiter)" do
        expected = "0.2.0 setup[::,==,--n] 2010-09-21 23:15:20 action==address:=search-n"
      ...
      it "should handle contexts with the delimiter (and change the delimiter)" do
        expected = "0.2.0 setup[::,==,--n] 2010-09-21 23:15:20 action==address :: server==market:eval=-n"
        actual = Appstats::Logger.entry_to_s("address", :server => 'market:eval=-n')
        actual.should == expected
      end

      it "should ignore spaces" do
        expected = "0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address search"
        actual = Appstats::Logger.entry_to_s("address search")
        actual.should == expected
      end
      
      it "should convert newlines in action" do
        expected = "0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address_-nsearch"
        actual = Appstats::Logger.entry_to_s("address_\nsearch")
        actual.should == expected
      end

      it "should convert newlines in context" do
        expected = "0.2.0 setup[:,=,-n] 2010-09-21 23:15:20 action=address_search : blah=some-nlong-nstatement"
        actual = Appstats::Logger.entry_to_s("address_search",:blah => "some\nlong\nstatement")
        actual.should == expected
      end
      
      it "should convert newlines based on the delimiter" do
        expected = "0.2.0 setup[::,==,--n] 2010-09-21 23:15:20 action==address:=--nsearch-n"
